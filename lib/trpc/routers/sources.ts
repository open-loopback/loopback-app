...
export const sourcesRouter = createTRPCRouter({
  getAll: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      // Verify project ownership (cached)
      const project = await cache.wrap(
        `project:${input.projectId}:owner`,
        async () => {
      ...
      return await cache.wrap(
        `project:${input.projectId}:sources`,
        async () => {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await cache.del(`project:${input.projectId}:sources`);
  ...
  regenerateToken: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Invalidate caches
      await cache.del(`project:${source.projectId}:sources`);
      await cache.del(`source:${input.id}`);
